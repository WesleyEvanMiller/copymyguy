...
# Output file
output_file="vmss_zone_report.txt"
echo "Generating VMSS zone report..." > "$output_file"
...
for subscription in "${subscriptions[@]}"; do
    ...
    for row in $(echo "${vmss_list}" | jq -c '.[]'); do
        ...
        instances_info=$(az vmss list-instances --resource-group $RESOURCE_GROUP --name $VMSS_NAME --query "[].{zone:zones, sku:sku.name}" --output json)
        instances_zones=$(echo "$instances_info" | jq -r '.[].zone | select(. != null) | unique | @csv' | tr -d '"')
        ...
        # Print VM size for each instance
        vm_sizes=$(echo "$instances_info" | jq -r '.[] | .sku')
        vm_sizes_unique=$(echo "${vm_sizes[@]}" | tr ' ' '\n' | sort -u | tr '\n' ' ')

        if [ ${#missing_zones[@]} -gt 0 ]; then
            echo "VMSS Name: $VMSS_NAME in Resource Group: $RESOURCE_GROUP is missing VMs in zones: ${missing_zones[*]}. VM Sizes: $vm_sizes_unique" >> "$output_file"
        else
            echo "VMSS Name: $VMSS_NAME in Resource Group: $RESOURCE_GROUP has VMs in all configured zones. VM Sizes: $vm_sizes_unique" >> "$output_file"
        fi
    done
done
