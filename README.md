...
for subscription in "${subscriptions[@]}"; do
    ...
    for row in $(echo "${vmss_list}" | jq -c '.[]'); do
        ...
        ZONES=$(echo "$row" | jq -r '.zones | @csv | if . == "null" then "" else . | split(",") | map(. | gsub("\""; "")) | .[] end')
...
for subscription in "${subscriptions[@]}"; do
    ...
    for row in $(echo "${vmss_list}" | jq -c '.[]'); do
        ...
        # List instances and their zones
        instances_zones=$(az vmss list-instances --resource-group $RESOURCE_GROUP --name $VMSS_NAME --query "[].zone" --output json | jq -r 'select(. != null) | .[]')

        # Check for VM presence in each configured zone
        missing_zones=()
        for zone in "${ZONES_ARRAY[@]}"; do
            if [[ ! " ${instances_zones[@]} " =~ " $zone " ]]; then
                missing_zones+=("$zone")
            fi
        done
